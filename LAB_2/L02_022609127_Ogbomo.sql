...
UPDATE 
    PRODUCTS
SET
    LIST_PRICE = LIST_PRICE + (LIST_PRICE * (2/100))
...
ELSIF(item.list_price >= ((avg_price - min_price) /2) AND item.list_price <= ((max_price - avg_price)/2)) THEN
fair_item := fair_item +1;
END IF;
END LOOP;

DBMS_OUTPUT.put_line('Cheap: '|| cheap_item || chr(10) ||'Fair: ' || fair_item || chr(10) || 'Expensive: '|| expensive_item);
